...
ArtifactKind 是 Android Gradle Plugin 中定义工件类型（Kind）的枚举类，包含三种基本类型：
- **SINGLE（单文件类型）**：整个文件作为整体处理，典型产物如 APK、JAR、AAR
- **MULTIPLE（多文件类型）**：包含多个独立文件，逐个处理，典型产物如 .class 文件集合、图片资源集合
- **DIRECTORY（目录类型）**：以整个文件夹作为处理单元，典型产物如 res 目录、assets 目录

### 结构图

```mermaid
flowchart TB
    subgraph ArtifactKind["ArtifactKind 枚举类"]
        DirectionLR
        SINGLE["SINGLE<br/>单文件"]
        MULTIPLE["MULTIPLE<br/>多文件"]
...
**❌ 错误：混淆 Kind 和 Type**
- Kind 是大类（单文件/多文件/目录），Type 是细分（如 CLASSES、JAR）
- 常见错误：在 Transform 中声明输出为 SINGLE，但实际处理多个文件

**❌ 错误：Transform 输入输出 Kind 不匹配**
- 如果输入是 MULTIPLE，输出也应该是 MULTIPLE
- 输入 SINGLE → 输出 SINGLE：正常
- 输入 SINGLE → 输出 MULTIPLE：需要特殊处理

### 设计哲学

ArtifactKind 的设计体现了"**关注点分离**"原则：
- **SINGLE**：处理简单，结果确定，适合最终产物（APK、AAR）
- **MULTIPLE**：处理灵活，适合中间产物（class 文件、资源文件）
- **DIRECTORY**：整体性强，适合需要保持目录结构的资源

这种分类让 Gradle 能够：
1. 正确地安排 Transform 的执行顺序
2. 高效地处理不同类型的工件
3. 提供清晰的 API 供开发者使用
...
### ★ 区分 SINGLE 和 MULTIPLE

查看以下产物，判断其 ArtifactKind：
1. `app/build/outputs/apk/debug/app-debug.apk` → **SINGLE**
2. `app/build/intermediates/javac/debug/classes/` 下的 .class 文件集合 → **MULTIPLE**
3. `app/src/main/res/` 目录 → **DIRECTORY**
4. `app/build/generated/res/resValues/debug/values/` 下的所有资源文件 → **MULTIPLE**

### ★★★ 设计一个 Transform

设计一个将 Kotlin 字节码转换为 Dex 格式的 Transform：
- 输入 Kind：MULTIPLE（多个 .class 文件）
- 输出 Kind：MULTIPLE（多个 .dex 文件）
- 需要考虑：如何分割 classes 以生成多个 dex

### ★★★★ Kind 转换的边界情况

讨论以下场景是否可行：
1. 输入 SINGLE → 输出 DIRECTORY：通常不可行，单文件无法变成目录
2. 输入 DIRECTORY → 输出 SINGLE：将目录内容打包成单个文件（可行，如 AAR）

---

## 面试热身

### Q1: ArtifactKind 有哪几种类型？
...
**A**: 处理最终产物，如 APK、AAR、JAR 等需要整体分发的文件。

### Q3: MULTIPLE 和 DIRECTORY 的区别？

**A**: 
- MULTIPLE：逐个文件处理，文件之间相互独立
- DIRECTORY：以目录整体处理，保持目录结构

### Q4: 如何在 Transform 中声明输入/输出的 Kind？

**A**: 使用 `TransformManager.registerTransform()` 时，通过 `ContentType` 参数指定Kind。

### Q5: 为什么不建议混合不同 Kind 的输入输出？

**A**: Gradle 的任务依赖图是基于 Kind 优化的，混合可能导致：
- 执行顺序混乱
- 增量构建失效
- 难以预测的构建错误
...
class DexTransform : Transform() {
    ...
    override fun getOutputTypes(): TransformOutputProvider {
        // 输出类型：DEX 文件
        return setOf(QualifiedContent.DefaultContentType.DEX)
    }
