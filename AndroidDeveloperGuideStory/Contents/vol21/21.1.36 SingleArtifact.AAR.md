...
SingleArtifact.AAR 是 Android Gradle Plugin 提供的 **Android Library Archive** 输出类型，表示库模块的构建产物。

### AAR 结构

```
example.aar
├── classes.jar         # Java 编译后的字节码
├── R.txt               # 资源 ID 映射表
├── res/                # 编译后的资源目录
├── AndroidManifest.xml # 合并后的清单文件
├── jni/                # 原生库（可选）
└── assets/             # 原始资源（可选）
```
...
| APK | 应用安装包 |
| BUNDLE | App Bundle |
| MERGED_MANIFEST | 合并后的清单 |

### 反模式与陷阱

1. **硬编码路径** → 使用 Artifacts API
2. **混淆 AAR 和 APK** → AAR 是库，APK 是应用
...
```kotlin
val aarFile = project.layout.artifacts
    .get(SingleArtifact.AAR)
```
...
**A**: JAR 仅包含字节码；AAR 额外包含 Android 资源、清单、原生库。

### Q2: 如何获取 AAR 工件？

**A**: 通过 `artifacts.get(SingleArtifact.AAR)`

### Q3: AAR 包含哪些内容？

**A**: classes.jar、R.txt、res/、AndroidManifest.xml、jni/、assets/
...
```kotlin
// 获取库模块的 AAR
val aarArtifact = project.layout.artifacts
    .getBuiltArtifacts(SingleArtifact.AAR)

// 获取 AAR 文件
val aarFile = aarArtifact.singleFile()
```
