...
App Bundle是现代Android应用发布的标准格式，建议所有通过Google Play分发的应用都使用Bundle。通过本章节的学习，应该理解Bundle的工作原理、优势和限制，以及何时需要使用APK替代。实际项目中，可以使用bundletool进行本地测试，体验Bundle带来的体积优化效果。

---

## 洛芙的小小日记本

今天黛琳教会了我App Bundle的魔法！原来开发者和用户之间隔着一个"Google Play分拣中心"——开发者送上的是一个"百宝箱"，但每个用户打开的却是"专属定制"。APK是"一言不合就全套"，Bundle是"按需分配刚刚好"。伊莎说的便当比喻太形象了——做好全家桶，每个人只吃自己那份。继续加油！✨

---

## 今日关键词

**SingleArtifact.BUNDLE**：Android Gradle API中的App Bundle构建工件类型

**App Bundle**：Google Play推荐的发布格式（.aab文件）

**APK**：Android安装包格式，最终可安装的应用包

**动态拆分**：Bundle根据设备配置动态生成APK的机制

**语言拆分**：App Bundle根据用户语言生成对应APK的机制

**ABI拆分**：App Bundle根据CPU架构生成对应APK的机制

**屏幕密度拆分**：App Bundle根据屏幕分辨率生成对应APK的机制

**动态功能模块**：App Bundle支持的可按需下载的功能模块

**bundletool**：Google提供的Bundle本地测试工具

**Play Core库**：处理动态功能模块下载的Android库

**SplitInstallManager**：Play Core库中管理动态模块安装的类

**bundleRelease**：生成Release版本App Bundle的Gradle任务

**Google Play分发**：App Bundle只能通过Play分发，不支持直接安装