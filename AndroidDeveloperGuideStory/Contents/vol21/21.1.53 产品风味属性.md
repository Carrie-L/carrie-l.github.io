...
ProductFlavorAttr 是 Android Gradle Plugin 中用于描述**产品风味（Product Flavor）**的属性类型。产品风味是构建变体的维度之一，用于区分同一应用的不同版本（如免费版/付费版、国内版/海外版）。
...
| paid | 付费版 | 完整功能 |
| china | 国内版 | 百度地图 |
| global | 海外版 | Google Maps |

### 与 BuildTypeAttr 的区别

- **BuildTypeAttr**: debug / release 构建类型
- **ProductFlavorAttr**: free / paid / china / global 产品风味
...
dependencies {
    ...
    paidImplementation("com.example:paid-feature:1.0")
}
```

### 反模式与陷阱

1. **在普通 implementation 中添加付费依赖** → 增大免费版体积
2. **混淆 BuildType 和 ProductFlavor** → 两者是不同维度

---

## 动手练习

### ★ 验证 ProductFlavorAttr

```bash
./gradlew app:dependencies --configuration freeDebugCompileClasspath
```

### ★★ 配置双维度产品风味

在 app/build.gradle 中配置多维度产品风味：
```kotlin
...
### Q1: ProductFlavorAttr 是什么？

**A**: 产品风味属性，用于区分 free、paid 等产品版本。

### Q2: productFlavors 和 buildTypes 的区别？

**A**: productFlavors 是版本维度，buildTypes 是构建类型维度。

### Q3: 如何实现免费版/付费版分离？

**A**: 使用 freeImplementation / paidImplementation

### Q4: 变体数量如何计算？

**A**: flavor数量 × buildType数量
