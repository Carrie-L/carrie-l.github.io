...
```kotlin
// 生成报告 Task
tasks.register<DefaultTask>("generateReport") {
    outputs.file(layout.buildDirectory.file("reports/report.json"))
    doLast {
        // 生成简单的 JSON 报告
    }
}
...
// 注册为 Artifact
artifacts.add("default", tasks.named("generateReport")) { artifact ->
    artifact.type = "build-report"
}
```
