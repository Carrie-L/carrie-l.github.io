...
希尔 grin（露出灿烂的笑容）：“区别可大了！旧版 API 想要追加点东西，得用各种 trick，比如修改 **sourceSets**（源文件集合，Gradle 用来告诉构建系统‘从哪里读取代码和资源’）、配置 **mergeResources**（资源合并任务）之类的。新版 API 直接给你一个统一的接口，想追加什么就追加什么。”
...
**答**：**SingleArtifact**（单一 Artifact）代表一个单一不可变的输出文件——就像一个密封的瓶子做好了就不能再改，比如最终的 APK 文件就是 SingleArtifact，你只能在构建完成后获取它，不能中途往里加东西。而 **Artifact.Appendable**（可追加 Artifact）允许在构建过程中向同一输出位置追加多个来源的文件，适用于资源、Assets、JniLibs 等需要合并的场景。Appendable 提供了 `additional()` 和 `from()` 方法来声明要添加的文件，系统会在构建时自动处理合并逻辑。
