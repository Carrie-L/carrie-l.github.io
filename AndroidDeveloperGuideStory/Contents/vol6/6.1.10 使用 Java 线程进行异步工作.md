...
**主线程（Main Thread）**：Android 应用启动时系统自动创建的线程，也称 UI 线程。负责处理所有用户输入事件和更新 UI 组件。Android 强制要求所有 UI 操作必须在主线程执行，否则可能引发不可预期的错误或崩溃。

**Thread**：Java 中实现多线程的基础类。通过继承 `Thread` 或实现 `Runnable` 接口来定义线程执行体。直接 `new Thread` 适用于一次性简单任务，但不建议在复杂场景使用，因为难以管理和获取返回值。

**Handler**：Android 中用于线程间通信的核心机制。与 Looper 配合工作：Looper 从消息队列取出消息，Handler 处理消息或发送消息到队列。子线程通过 Handler 向主线程发送消息来实现 UI 更新。

**Looper**：每个线程的消息循环器，每个线程最多只有一个 Looper。调用 `Looper.prepare()` 初始化消息队列，`Looper.loop()` 开始循环处理消息。主线程自带 Looper一直在运行，这也是主线程能够响应各种事件的原因。

**ExecutorService**：Java 并发包 `java.util.concurrent` 提供的线程池接口，比直接 new Thread 更适合管理多线程。支持任务提交、线程复用、线程池关闭等操作。

**ThreadPoolExecutor**：线程池的核心实现类，支持自定义核心线程数、最大线程数、空闲线程存活时间、任务队列容量及拒绝策略。推荐使用此类代替 Executors 工厂方法，以避免无界队列导致的 OOM 问题。

**Runnable 与 Callable**：定义线程执行体的接口。`Runnable` 的 `run()` 方法无返回值；`Callable` 的 `call()` 方法有返回值，需与 `Future` 配合获取结果。

**synchronized**：Java 内置的同步关键字，保证同一时刻只有一个线程能执行被保护的方法或代码块。是实现线程安全最简单的方式，但会阻塞线程，开销较大。

**volatile**：确保变量的可见性（一个线程修改后其他线程立即可见）和有序性（禁止指令重排序）。不保证原子性，`count++` 等复合操作在多线程下仍不安全。

**AtomicInteger / Atomic类**：基于 CAS 无锁算法实现的原子类，提供线程安全的单一变量操作。性能优于 synchronized，适用于计数器等简单场景。
...
flowchart TB
    ...
    subgraph 线程池
        TP["ThreadPoolExecutor<br/>corePoolSize<br/>maxPoolSize<br/>有界队列<br/>拒绝策略"]
    end
...
线程模型全景图：主线程通过 Handler+Looper 处理 UI 事件和消息，线程池管理后台线程执行耗时任务，任务完成后通过 Handler 将结果投递回主线程更新界面。

### 反模式与陷阱

1. **主线程阻塞（ANR）**：在主线程执行网络请求、大文件读取或复杂计算，导致 UI 无响应超过 5 秒触发 ANR 对话框。所有耗时操作必须移到子线程。

2. **子线程更新 UI**：在子线程直接调用 `textView.setText()` 等 UI 操作会抛出 `CalledFromWrongThreadException`。Android 的 UI 组件只能在主线程访问。

3. **直接使用 Executors 工厂方法**：`Executors.newFixedThreadPool()` 内部使用无界队列，当任务生产速度大于消费速度时，队列无限膨胀导致 OOM。生产环境应使用 `ThreadPoolExecutor` 手动创建有界队列。

4. **线程池未关闭**：`ExecutorService` 创建后若不调用 `shutdown()` 或 `shutdownNow()`，线程池会一直运行，导致资源泄漏。使用 try-with-resources 或在 `onDestroy()` 中确保关闭。

5. **竞态条件**：`count++` 在多线程下不是原子操作，两个线程可能同时读取、同时加 1、同时写回，导致结果小于预期。使用 `synchronized` 或 `AtomicInteger` 保护共享变量。

6. **volatile 的误用**：`volatile` 只保证可见性和有序性，不保证原子性。`volatile boolean flag; flag = !flag;` 这样的复合操作仍不是线程安全的。

7. **忽略拒绝策略**：当线程池队列满且最大线程数都在忙碌时，新任务会被拒绝。若未设置合适的拒绝策略，任务可能无声丢失。使用 `CallerRunsPolicy` 可兜底但需注意对主线程的影响。

### 设计哲学

**职责分离**：主线程专职负责 UI 渲染和用户交互，后台线程专职执行耗时操作。这遵循了单一职责原则，让每种线程各司其职。

**线程复用**：线程的创建和销毁有较大开销，线程池通过复用已有线程避免频繁创建销毁，提升性能。这体现了资源复用的设计思想。

**异步与同步的桥梁**：Handler+Looper 机制建立了子线程与主线程之间的通信桥梁，让异步结果以同步方式更新 UI。`View.post()` 和 `runOnUiThread()` 是这一哲学的便捷封装。

**线程安全不是加锁那么简单**：过度加锁会严重影响性能，而盲目依赖 volatile 或 Atomic 类可能在复杂场景下失效。正确的做法是根据场景选择合适的同步策略——简单计数器用 Atomic 类，复合操作用 synchronized，涉及复杂状态用显式 Lock。

**现代Android的演进方向**：Kotlin 协程将线程切换封装为 suspend/resume 机制，让异步代码以同步风格编写，大幅降低了多线程编程的复杂度。但理解底层线程模型仍是 Android 开发者的必备基础。
...
- **目标**：掌握创建和启动线程的基本方法
- **步骤**：
  1. 创建一个类继承 `Thread`，重写 `run()` 方法，打印当前线程名称
  2. 在 `main()` 方法中创建该线程实例并调用 `start()`
  3. 观察输出中主线程名称与子线程名称的区别
- **验收标准**：输出包含 "Thread-" 开头的子线程名称，且与主线程名称不同
- **提示代码**：
  ```java
...
**Task 2: Runnable 与线程返回值**
- **目标**：理解 Runnable 接口，学会获取线程执行结果
- **步骤**：
  1. 使用 `Executors.newSingleThreadExecutor()` 创建单线程线程池
  2. 提交一个 `Callable<String>` 返回 "Hello from thread"
  3. 调用 `Future.get()` 获取返回值并打印
  4. 记得关闭线程池
- **验收标准**：成功打印 Callable 的返回值，线程池被正确关闭
- **提示代码**：
  ```java
  ExecutorService executor = Executors.newSingleThreadExecutor();
  Future<String> future = executor.submit(() -> "Hello from thread");
  System.out.println(future.get()); // 会阻塞等待结果
  executor.shutdown();
  ```

**Task 3: Handler 消息机制**
- **目标**：理解 Handler 发送和处理消息的完整流程
- **步骤**：
  1. 在主线程创建 `Handler(Looper.getMainLooper())`
  2. 在子线程发送一条 `Message`，`what=1, obj="data"`
  3. 在 Handler 的 `handleMessage()` 中打印消息内容
  4. 使用 `Handler.post()` 方式实现相同功能，比较两者区别
- **验收标准**：主线程能正确接收并处理子线程发送的消息
- **提示代码**：
  ```java
  ...
  // 子线程中: mainHandler.sendMessage(msg);
  ```

**Task 4: 手动创建线程池**
- **目标**：掌握 `ThreadPoolExecutor` 的正确创建方式
- **步骤**：
  1. 创建 `ThreadPoolExecutor`，参数：corePoolSize=2, maxPoolSize=4, queueCapacity=10
  2. 使用 `CallerRunsPolicy` 作为拒绝策略
  3. 提交 15 个任务（每个打印线程名和任务编号）
  4. 观察超过队列容量时的拒绝策略行为
- **验收标准**：队列满后触发 CallerRunsPolicy，调用者线程（main）也参与执行任务
- **提示代码**：
  ```java
...
**Task 5: runOnUiThread 与 View.post**
- **目标**：掌握两种切回主线程的便捷方式
- **步骤**：
  1. 在 Activity 中创建一个按钮，点击时启动子线程
  2. 子线程模拟耗时操作（`Thread.sleep(1000)`）后，分别用 `runOnUiThread()` 和 `View.post()` 更新 UI
  3. 观察两种方式的效果是否一致
- **验收标准**：子线程执行完毕后，UI 被正确更新，按钮文字或 TextView 内容发生预期变化
- **提示代码**：
  ```java
  // 方式1
  runOnUiThread(() -> textView.setText("更新了"));
  // 方式2
  textView.post(() -> textView.setText("更新了"));
  ```

**Task 6: 线程安全——synchronized vs AtomicInteger**
- **目标**：理解竞态条件，掌握两种线程同步方式
- **步骤**：
  1. 创建不安全的计数器类，并发访问 1000 次，验证结果小于 1000
  2. 用 `synchronized` 修饰 `increment()` 方法，验证结果等于 1000
  3. 改用 `AtomicInteger`，验证结果也等于 1000
  4. 对比两种方式的性能差异
- **验收标准**：
  - 不安全版本结果 < 1000（复现竞态条件）
  - synchronized 和 AtomicInteger 版本结果均 = 1000
- **提示代码**：
  ```java
  // AtomicInteger 版本
  AtomicInteger count = new AtomicInteger(0);
  // 多线程中: count.incrementAndGet();
  ```

**Task 7: 完整异步网络请求流程**
- **目标**：综合运用线程池和 Handler，实现完整的异步任务流程
- **步骤**：
  1. 创建 `ThreadPoolExecutor`（core=2, max=4）
  2. 提交网络请求任务（用 `Thread.sleep(2000)` 模拟）
  3. 请求完成后通过 `Handler` 发送结果到主线程
  4. 在主线程更新 UI 显示请求结果
  5. 处理请求失败的情况（用 `try-catch` 模拟异常）
- **验收标准**：模拟请求成功时 UI 显示成功信息，失败时显示错误信息，且 UI 全程保持响应
- **提示代码**：
  ```java
  executor.submit(() -> {
      try {
          ...
          mainHandler.sendMessage(Message.obtain().apply {
              what = SUCCESS; obj = result;
          });
      } catch (Exception e) {
          mainHandler.sendMessage(Message.obtain().apply {
              what = ERROR; obj = e.getMessage();
          });
      }
  });
  ```
...
**Task 8: 分析 ANR 产生原因**
- **目标**：理解 ANR 的触发条件，学会排查和避免 ANR
- **步骤**：
  1. 在主线程的 `onCreate()` 中执行 `Thread.sleep(6000)`，观察是否触发 ANR
  2. 将耗时操作移到子线程，验证 ANR 不再出现
  3. 使用 `StrictMode.ThreadPolicy` 辅助检测主线程耗时操作
- **验收标准**：原始代码触发 ANR，修复后不再触发，`StrictMode` 报告被正确读取
- **提示代码**：
  ```java
...
**Q1: Android 中为什么禁止在子线程更新 UI？主线程都做了哪些事？**

主线程也称为 UI 线程，负责处理所有用户交互事件（点击、滑动等）和 View 渲染绘制。如果子线程可以随意更新 UI，多个线程同时操作同一个 View 可能导致界面错乱、数据不可预期等问题。Android 采用单线程 UI 模型，强制所有 UI 操作必须在主线程执行，由 Looper 循环处理消息队列确保线程安全。主线程还负责测量（measure）、布局（layout）、绘制（draw）等 View 层级结构的核心流程。

**Q2: `Thread.start()` 和 `Thread.run()` 有什么区别？为什么必须调用 `start()`？**

`Thread.run()` 只是普通的方法调用，会在当前线程（调用者线程）中同步执行 `run()` 方法的内容，不会创建新线程。而 `Thread.start()` 会指示 JVM 启动新线程，在新线程中异步执行 `run()` 方法。必须调用 `start()` 是因为 `run()` 本身没有创建线程的能力——它只是一个普通的实例方法，逻辑上可以被任何线程调用，但只有 `start()` 才会真正触发系统创建新线程。

**Q3: `Handler`、`Looper`、`MessageQueue` 三者的关系是什么？一条消息从发送到处理的完整流程是怎样的？**

`Looper` 是消息循环器，每个线程只能有一个，它持有 `MessageQueue`（消息队列）并不断从队列中取出消息。`Handler` 是消息发送和处理的中介，可以发送消息到任意线程的队列（只要持有该线程的 Looper 引用），同时负责处理自己发出的消息。流程：1）子线程通过 `handler.sendMessage()` 将 Message 加入主线程的 MessageQueue；2）主线程的 Looper.loop() 从队列中取出消息；3）Looper 将消息分发给创建它的 Handler；4）Handler.handleMessage() 在主线程执行，完成 UI 更新。

**Q4: `Executors.newFixedThreadPool()` 有什么潜在问题？为什么不推荐直接使用 Executors 工厂方法创建线程池？**

`Executors.newFixedThreadPool()` 内部使用 `LinkedBlockingQueue` 作为任务队列，默认容量是 `Integer.MAX_VALUE`，相当于无界队列。当任务提交速度持续大于处理速度时，队列会无限膨胀，最终导致 OutOfMemoryError。此外，某些 Executors 方法（如 `newCachedThreadPool()`）的最大线程数无限制，在高负载场景下可能创建过多线程。正确做法是使用 `ThreadPoolExecutor` 手动创建线程池，明确指定有界队列容量和拒绝策略，防止资源耗尽。

**Q5: `synchronized` 和 `AtomicInteger` 都能保证线程安全，它们有什么区别？各自适合什么场景？**

`synchronized` 是悲观锁，获取锁失败时线程会阻塞等待，适用于复合操作（读-改-写，如 `count++`）和多操作场景，开销较大但使用简单。`AtomicInteger` 基于 CAS 无锁算法，实现 `incrementAndGet()` 等原子操作，线程不阻塞但可能产生"忙等待"（自旋重试），适合单一变量的原子操作，性能通常优于 synchronized。复杂场景（如需要保护多个变量的不变式）应使用 synchronized 或显式 Lock。
