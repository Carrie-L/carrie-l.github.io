...
**Q1: 为什么 Android 要限制应用在 Manifest 中声明隐式广播接收器？这样做解决了什么问题？**

**Q2: 哪些广播是豁免的（可以在 Manifest 中声明）？请列举至少 5 个并说明各自的触发场景。**

**Q3: 如果一个应用需要响应非豁免的隐式广播，应该如何实现？请给出两种替代方案并比较其优劣。**

**Q4: 显式广播（Explicit Broadcast）和隐式广播（Implicit Broadcast）的区别是什么？在 Android O 之后，为什么推荐使用显式广播？**

**Q5: 假设一个应用同时在 Manifest 和代码中注册了同一个广播接收器，系统会如何处理？请解释原因。
---
