...
**唤醒锁（Wake Lock）** — Android 系统中的一种机制，用于阻止设备进入休眠状态。持有唤醒锁可以保证 CPU 或屏幕在设备应该休眠时仍然保持工作状态，但会持续消耗电量。分为 PARTIAL_WAKE_LOCK（仅保持 CPU）和完整唤醒锁（SCREEN_*_WAKE_LOCK，保持屏幕）。

**哪些 API 会偷偷创建唤醒锁** — 多个系统服务在执行特定操作时会自动持有 WakeLock：SensorManager 在 registerListener() 时持有、AlarmManager 在 setExact() 精确闹钟时持有、LocationManager 在持续位置更新时持有，WifiManager 在持有 WifiLock 时阻止 WiFi 进入低功耗模式。这些 WakeLock 对开发者不可见，但会显著影响电池续航。

**dumpsys power** — 系统调试命令，列出当前所有活跃的 WakeLock 及其持有者、UID、持有时长。用于分析是谁在背后消耗电量。

**dumpsys batteryinfo** — 详细电池统计数据，显示各进程的 WakeLock 持有时长、唤醒次数等，用于量化电量消耗来源。

**正确取消注册** — 持有 WakeLock 的系统 API 必须在对应生命周期回调中释放。SensorManager.registerListener() 后在 onPause()/onStop() 中调用 unregisterListener()；WifiLock 在不需要时调用 release()。

#### 结构图

```mermaid
graph TD
    A[SensorManager.registerListener()] --> B[自动持有 WakeLock<br/>保证传感器持续供电]
    A --> C[Activity onPause/onStop]
    C --> D[unregisterListener()]
    D --> E[WakeLock 释放]
    F[AlarmManager.setExact()] --> G[内部持有 WakeLock<br/>精确时刻唤醒设备]
    G --> H[到达指定时刻]
    H --> I[设备唤醒执行]
    J[LocationManager.requestLocationUpdates()] --> K[持有 WakeLock<br/>保持定位模块工作]
    K --> L[停止请求 / onPause]
    L --> M[removeUpdates()]
    M --> N[WakeLock 释放]
```
...
```mermaid
graph LR
    A[dumpsys power] --> B[列出活跃 WakeLock]
    B --> C{系统API还是<br/>自己代码?}
    C -->|系统API| D[对照文档定位来源]
    C -->|自己代码| E[检查注册/取消配对]
    D --> F[无法直接优化]
    E --> G[在生命周期回调<br/>添加取消注册]
```

#### 反模式与陷阱

**1. 只注册不取消** — SensorManager.registerListener() 后不在 onPause()/onDestroy() 中调用 unregisterListener()，导致 Activity 退出后 WakeLock 仍然持有。是最常见的电量流失原因之一。

**2. WifiLock 不释放** — WifiManager.createWifiLock() 后忘记调用 release()，即使不需要网络，WiFi 也会持续工作，耗电巨大。

**3. 盲目信任系统 API 自动优化** — 系统 API 持有 WakeLock 是为了确保功能正常，但与来源无关，只要持有就会耗电。

**4. 在不必要的时候持有完整 WakeLock** — SCREEN_BRIGHT_WAKE_LOCK 会阻止屏幕变暗，长时间持有会快速耗光电量。

#### 设计哲学

**最小权限原则在 WakeLock 中的应用** — 理解系统服务的 WakeLock 持有机制，才能在应用层面做出明智取舍。对于传感器、位置、WiFi 等常见耗电来源，只在必要时请求持续服务，不需要时立即释放。

**可观测性优先** — 遇到无法解释的电量消耗时，第一步永远是 dumpsys power 和 dumpsys batteryinfo。在定位到具体 WakeLock 来源之前，不要盲目优化。


---
#### 🏕️ 动手练习

**目标**：掌握识别和解决系统 API 导致的 WakeLock 持有问题。

**Task 1：使用 dumpsys power 分析 WakeLock（★）**  
目标：在设备上运行 Demo App，使用 dumpsys power 观察 WakeLock 列表。  
步骤：创建 SensorManager Demo，在 onCreate() 中注册传感器但不取消注册。运行后执行 `adb shell dumpsys power`，找到你的 App 持有的 WakeLock 记录。  
验收标准：能在 dumpsys power 输出中定位到自己的 App 名称和 WakeLock 类型。

**Task 2：复现传感器 WakeLock 不释放问题（★★）**  
目标：创建 Activity 注册传感器但不 unregisterListener()，验证 Activity 退出后 WakeLock 仍在。  
验收标准：Activity 退出后 dumpsys power 中仍能看到该 App 的 WakeLock 记录。

**Task 3：修复传感器注册——配对取消注册（★★）**  
目标：在 Task 2 基础上，在 onPause() 中添加 unregisterListener()。验证修复后 WakeLock 被正确释放。  
验收标准：Activity 退出后 dumpsys power 中不再有该 App 的 WakeLock。

**Task 4：使用 dumpsys batteryinfo 分析电量消耗（★★★）**  
目标：使用 batteryinfo 命令分析修复前后的耗电差异。  
验收标准：能说出修复前后 WakeLock 持有时长的具体数值差异。

**Task 5：定位 WifiLock 的持有者（★★★）**  
目标：创建持有 WifiLock 的 App，用 dumpsys power 找到它。  
验收标准：能在 dumpsys 输出中定位到 WifiLock。

**Task 6：AlarmManager 精确闹钟的 WakeLock（★★★★）**  
目标：设置 setExact() 精确闹钟，用 dumpsys 观察 AlarmManager 的 WakeLock 持有。  
验收标准：能描述闹钟触发前后 WakeLock 的生命周期变化。

**Task 7：编写电量自检工具（★★★★★）**  
目标：综合运用 dumpsys 命令，编写脚本自动分析指定 App 的 WakeLock 持有情况并输出报告。  
验收标准：能输出一份包含 WakeLock 类型、持有时长、唤醒次数的格式化报告。

**面试热身**

**Q1**：Android 中哪些常见的系统 API 会在后台持有 WakeLock？  
**A**：SensorManager（registerListener）、AlarmManager（setExact/setAlarmClock）、LocationManager（持续位置更新）、WifiManager（WifiLock）、PowerManager（newWakeLock）。共同点：请求持续服务时自动持有，不释放则持续耗电。

**Q2**：如何判断一个 WakeLock 是系统 API 创建的还是自己代码创建的？  
**A**：通过 WakeLock Tag 区分。系统级 Tag 以 "Service" 结尾；自定义 Tag 包含包名或类名。还可通过 dumpsys power 的 UID 列定位具体 App。

**Q3**：为什么 SensorManager.registerListener() 后必须取消注册？  
**A**：registerListener() 内部会持有 PARTIAL_WAKE_LOCK 保证传感器持续供电。不取消注册则 Activity 退出后 WakeLock 仍被持有，CPU 无法休眠，电量持续流失。

**Q4**：dumpsys power 和 dumpsys batteryinfo 的区别是什么？  
**A**：dumpsys power 列出当前所有活跃的 WakeLock，适合实时分析。dumpsys batteryinfo 提供历史统计数据（持有总时长、唤醒次数），适合分析一段时间内的耗电情况。

**Q5**：WifiLock 和 WiFi 扫描有什么区别？  
**A**：WifiLock 阻止 WiFi 进入低功耗模式，适用于后台下载等需要持续网络的场景。WiFi 扫描是短时搜索操作不需要 WifiLock。WifiLock 持有时耗电显著，不使用时必须 release()。

**参考实现要点**

1. **优先使用 WorkManager 替代手动 WakeLock** — WorkManager 内部已处理 WakeLock 持有和释放策略，能在满足约束条件时自动休眠。对于周期性后台任务，WorkManager 是比 WakeLock + AlarmManager 更优的选择。

2. **传感器注册必须成对出现** — registerListener() 和 unregisterListener() 必须配对。常见做法是在 onResume()/onPause() 或 onCreate()/onDestroy() 中配对。

3. **WifiLock 需要显式 release()** — WifiLock 通过 createWifiLock() 显式创建，必须在不需要时显式调用 release()。持有时 WiFi 模块全功率运行，耗电非常明显。

4. **dumpsys 是分析问题的起点** — 遇到无法解释的电量消耗时，第一步永远是 dumpsys power 和 dumpsys batteryinfo。

5. **考虑用 FusedLocationProviderClient 替代原始 LocationManager** — FusedLocationProvider 在底层做了更多电池优化，能在不需要位置时更快速地释放 WakeLock。
