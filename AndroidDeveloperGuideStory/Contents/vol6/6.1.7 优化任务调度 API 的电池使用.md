...
黛琳补充道：“这就是第一个优化原则——选择最优的约束条件。**RequiresCharging()** 告诉系统：只有在电池正在充电时才运行这个任务。**NetworkType.UNMETERED()** 告诉系统：只有连接上WiFi（不计量收费的网络）时才运行。还有一个常用的叫**NetworkType.CONNECTED()**，它允许在任何网络连接下执行——包括移动数据，但相应地也更耗电。”
...
洛芙举手提问："黛琳，我注意到代码里有个OutOfQuotaPolicy，这个是做什么的？"

"好问题！"黛琳点头，"你观察得很仔细。OutOfQuotaPolicy是当加急任务的'配额'用完时的处理策略--你可以把它理解为'每天允许加急的任务数量是有限制的'。当配额用完时，你可以选择两种策略：一种是RUN_AS_NON_EXPEDITED_WORK_REQUEST，也就是把加急任务降级为普通任务；另一种是DISCARD_DISCARD_WORK_REQUEST，也就是直接丢弃这个任务。"

"配额是系统定的吗？"希尔问。

"对，"黛琳说，"系统会根据App的使用情况和电池状态动态调整配额。如果你经常滥用加急任务，配额就会被降低。所以最安全的做法是使用RUN_AS_NON_EXPEDITED_WORK_REQUEST，这样就算配额用完了，任务至少还能以普通方式执行。"
...
黛琳点头："没错。这也是我们今天讲的所有内容的核心--做一个'好公民'App，尊重用户的电池，尊重系统的资源。用户用得开心，App才能活得久。"

伊莎好奇地问："对了，你之前提到的Android系统省电机制，除了Doze Mode还有别的吗？我记得关键词里好像有App Standby什么的。"

"问得好！"黛琳说，"Android有好几层省电机制。Doze Mode是当手机屏幕关闭、不动了一段时间后，系统会进入的一种深度睡眠状态，在这个状态下会暂停大部分后台任务；而App Standby则是针对单个App的--如果一个App很长时间没有被用户打开，系统就会把它放进'Standby状态'，限制它的后台活动。这两个机制都是为了延长电池寿命，但方向不同：Doze是全局的，App Standby是个别的。"

"那Restricted Standby Bucket和App Standby是一回事吗？"洛芙问。

"类似，但不完全一样，"黛琳解释，"App Standby是一种温和的提醒，告诉你'这个App很久没用了'；而Restricted Standby Bucket是更严格的惩罚--只有当你的App经常超时、消耗过多资源时才会触发。一旦被放进Restricted Bucket，系统会大幅削减你的后台任务执行次数，这个影响比普通Standby要严重得多。"
