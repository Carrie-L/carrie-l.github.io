...
---
## tailwind 操作方法
2025/09/15 新增本地 tailwind.css

1. 在根目录下的 tailwind.config.js 将文件夹目录添加进去：
```js
content: [
    './_includes/**/*.html',
    './_layouts/**/*.html',
...
```shell
npx --package=tailwindcss tailwindcss -i ./tailwind-input.css -o ./assets/css/tailwind.css --minify --config ./tailwind.config.js
```

然后它会在 /assets/css/ 下自动新建 tailwind.css，这个文件是自动创建的，所以不要修改它，要改样式就改根目录下的 tailwind-input.css

3. 把html里的 cdn tailwindcss
...
4. 在 html 头部添加：
(把html粘贴到_开头的目录文件夹里， 如 _Android/compose layout.html)

```
---
layout: bare
title: "Jetpack Compose 布局速通指南"
date: 2025-09-15
